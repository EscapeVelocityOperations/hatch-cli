...
type Deps struct {
	...
	RebuildApp   func(token, slug string) error
...
// NewCmd returns the rebuild command.
func NewCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "rebuild [slug]",
		Short: "Rebuild an application from its git repository",
		Long:  "Trigger a rebuild of a Hatch application from its git repository. Requires confirmation unless --yes is provided.",
...
func runRebuild(cmd *cobra.Command, args []string) error {
	...
	if !yesFlag && !deps.Confirm(fmt.Sprintf("Rebuild %s from git?", slug)) {
		ui.Info("Cancelled.")
		return nil
	}

	sp := ui.NewSpinner(fmt.Sprintf("Rebuilding %s from git repository...", slug))
	sp.Start()
	err = deps.RebuildApp(token, slug)
	sp.Stop()

	if err != nil {
		return fmt.Errorf("rebuilding app: %w", err)
	}

	ui.Success(fmt.Sprintf("Rebuild triggered for %s", slug))
	ui.Info(fmt.Sprintf("Logs: hatch logs %s", slug))
	return nil
}
...
func resolveSlug(args []string) (string, error) {
	...
	if !deps.HasRemote("hatch") {
		return "", fmt.Errorf("no app specified and no hatch git remote found. Usage: hatch rebuild <slug>")
	}
