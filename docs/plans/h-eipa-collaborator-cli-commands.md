...
- [x] T-008 — Wired `collab.NewCmd()`/`invite.NewCmd()` into `cmd/root/root.go` (alphabetical placement). Smoke-tested the real built binary: `--help` shows both groups with the secrets-trust copy, and the not-logged-in error path works end-to-end (verified it exits the same way — 0, not 1 — as the pre-existing `domain list` command; a repo-wide quirk, not something to fix here).
- [x] T-009 — Full suite gate: `go build`, `go vet`, `gofmt -l` clean on all touched files, `golangci-lint` clean relative to house standard (the only findings are the pre-existing repo-wide unchecked-`MarkFlagRequired` pattern, confirmed identical in `domain.go`), `go test ./... -race` (634 passed, no regressions). PR next.
